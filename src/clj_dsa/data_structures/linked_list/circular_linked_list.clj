(ns clj-dsa.data-structures.linked-list.circular-linked-list
...
(deftype CLL [^:volatile-mutable head ^:volatile-mutable tail]
  ICLL
  (set_head
    [_ h]
    (set! head h))
    ...
    (when head
      (loop [current-head head]
        (let [current-head-data (.get_data current-head)
          ...
          (when (not= head current-head-address)
